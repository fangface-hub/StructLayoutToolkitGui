...
class PacketDataEditorWindow(tk.Toplevel):
    ...
    def _load_dir_setting(self) -> Path | None:
        """Return PacketDataEditor's stored data directory, if any."""
        settings_file = self._settings_file()
        if not settings_file.is_file():
            return None
        try:
            settings = json.loads(settings_file.read_text(encoding="utf-8"))
            directory = settings.get(self.SETTINGS_SECTION, {}).get("data_dir")
    ...
    def _remember_data_dir(self, path: str | Path) -> None:
        ...
        settings.setdefault(self.SETTINGS_SECTION,
                            {})["data_dir"] = selected_dir.as_posix()
        try:
        ...
        if packet is not None and packet.complete:
            try:
                ...
                if self.struct_layout is not None:
                    instance = decode(self.struct_layout, packet.data)
                    self.struct_instance = self._plugin_manager().apply_decode(
                        instance)
    ...
    def _on_detail_value_finished(self, event: tk.Event) -> None:
        ...
        try:
            ...
            instance = decode(self.struct_layout, packet.data)
            self.struct_instance = self._plugin_manager().apply_decode(instance)
